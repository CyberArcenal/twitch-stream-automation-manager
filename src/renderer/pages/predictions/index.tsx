...
const PredictionsPage: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      alert("Failed to create prediction. Make sure you have the required scope.");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleResolve = async (predictionId: string, winningOutcomeId: string) => {
    if (!confirm("Resolve prediction with this outcome?")) return;
    try {
    ...
    <div className="p-6 space-y-6 bg-[var(--background-color)] min-h-screen">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[var(--text-secondary)]">Create and manage channel point predictions</p>
        </div>
        <button
      ...
      {/* Active Predictions */}
      <div>
        <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Active Predictions</h2>
        {predictions.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {predictions?.map((pred) => (
              <div key={pred.id} className="bg-[var(--card-bg)] rounded-xl border border-[var(--card-bg)] p-5">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-lg font-semibold text-[var(--text-primary)]">{pred.title}</h3>
                ...
                <div className="mt-4 space-y-3">
                  {pred.outcomes.map((outcome) => (
                    <div key={outcome.id} className="bg-[#2a2a2e]/50 rounded-lg p-3">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[var(--text-primary)] font-medium">{outcome.title}</span>
                        <div className="flex gap-3 text-sm">
                          <span className="text-[var(--text-secondary)]">{outcome.users} users</span>
                          <span className="text-[#9147ff] font-mono">{outcome.channel_points.toLocaleString()} pts</span>
                        </div>
                      </div>
                      {pred.status === "ACTIVE" && (
                        <button
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-[var(--card-bg)] rounded-xl w-full max-w-lg p-6 border border-[var(--card-bg)] max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-semibold text-[var(--text-primary)] mb-4">Create Prediction</h3>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-[var(--text-secondary)] mb-1">Outcomes (2–10 options)</label>
              ...
              <div>
                <label className="block text-sm text-[var(--text-secondary)] mb-1">Prediction Window (seconds)</label>
                <select
                  value={formData.predictionWindow}
                  onChange={(e) => setFormData({ ...formData, predictionWindow: parseInt(e.target.value) })}
                  className="w-full bg-[var(--background-color)] border border-[var(--card-bg)] rounded px-3 py-2 text-[var(--text-primary)]"
                >
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button onClick={() => setShowCreateModal(false)} className="px-4 py-2 bg-[#2a2a2e] rounded-md">
                Cancel
              </button>
              <button onClick={handleCreate} disabled={creating} className="px-4 py-2 bg-[#9147ff] rounded-md disabled:opacity-50">
