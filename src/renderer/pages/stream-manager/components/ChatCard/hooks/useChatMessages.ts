...
export const useChatMessages = (channelName?: string, currentUser?: string) => {
  ...
  const lastUserMessage = useRef<Map<string, { text: string; timestamp: number }>>(new Map());

  const isDuplicateMessage = (user: string, text: string, timestamp: number, isFromMe?: boolean): boolean => {
  ...
  const loadRecentMessages = async () => {
    ...
    if (res?.status && res.data) {
      const formatted = res.data.map(m => ({ ...m, id: m.messageId || m.id }));
  ...
  useEffect(() => {
    const handleMessage = (rawMsg: any) => {
      ...
      if (isDuplicateMessage(msg.user, msg.message, msgTime, msg.isFromMe)) return;
      setMessages(prev => {
      ...
      if (res?.status && res.data) {
        setMessages(prev => {
          const newMsgs = res.data.filter(m => {
            ...
            if (isDuplicateMessage(m.user, m.message, msgTime, m.isFromMe)) return false;
            return true;
          });
          if (newMsgs.length === 0) return prev;
          const formatted = newMsgs.map(m => ({ ...m, id: m.messageId || m.id }));
    ...
    try {
      ...
      if (res.status) {
        const updateFn = (msg: ChatMessage) =>
          msg.id === msgId ? { ...msg, message: '[Message removed]', isDeleted: true } : msg;
